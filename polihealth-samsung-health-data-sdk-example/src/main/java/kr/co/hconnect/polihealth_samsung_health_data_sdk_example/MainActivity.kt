package kr.co.hconnect.polihealth_samsung_health_data_sdk_example
...
import kr.co.hconnect.polihealth_samsung_health_data_sdk.HealthDataSdkException
import kr.co.hconnect.polihealth_samsung_health_data_sdk.PolihealthSamsungHealthDataSdk
import kr.co.hconnect.polihealth_samsung_health_data_sdk_example.ui.theme.PolihealthSamsungHealthDataSdkTheme
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            PolihealthSamsungHealthDataSdkTheme {
                MainScreen()
            }
        }
    }
 ...
 * (polihealth-samsung-health-data-sdk/docs/samsung-health-integration.md 11절).
 */
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Polihealth Samsung Health Data SDK Example") },
        ...
        Column(
            ...
            Row(
                ...
                HealthActionButton(
                    ...
                    runTask("권한 확인") {
                        val granted = PolihealthSamsungHealthDataSdk.checkHealthDataPermission(context)
                ...
                HealthActionButton(
                    ...
                    runTask("권한 요청") {
                        val granted = PolihealthSamsungHealthDataSdk.requestHealthDataPermission(target)
                        permissionState = granted
            ...
            Row(
                ...
                HealthActionButton(text = "걸음수", modifier = Modifier.weight(1f), enabled = !loading) {
                    runTask("걸음수") {
                        PolihealthSamsungHealthDataSdk.readHealthDataSteps(context).map { "[걸음수] $it" }
                            .ifEmpty { listOf("[걸음수] 데이터 없음") }
                    }
                }
                HealthActionButton(text = "심박수", modifier = Modifier.weight(1f), enabled = !loading) {
                    runTask("심박수") {
                        PolihealthSamsungHealthDataSdk.readHealthDataHeartRate(context).map { "[심박수] $it" }
            ...
            Row(
                ...
                HealthActionButton(text = "산소포화도", modifier = Modifier.weight(1f), enabled = !loading) {
                    runTask("산소포화도") {
                        PolihealthSamsungHealthDataSdk.readHealthDataBloodOxygen(context).map { "[산소포화도] $it" }
                            .ifEmpty { listOf("[산소포화도] 데이터 없음") }
                    }
                }
                HealthActionButton(text = "피부온도", modifier = Modifier.weight(1f), enabled = !loading) {
                    runTask("피부온도") {
                        PolihealthSamsungHealthDataSdk.readHealthDataSkinTemperature(context).map { "[피부온도] $it" }
