...
```kotlin
val granted = SamsungServerSdk.checkHealthDataPermission(context)   // Map<String, Boolean>
if (granted.values.any { !it }) {
    SamsungServerSdk.requestHealthDataPermission(activity)
}
val steps = SamsungServerSdk.readHealthDataSteps(context, days = 7)          // List<Map<String, Any>>
val heartRate = SamsungServerSdk.readHealthDataHeartRate(context, days = 7)
val bloodOxygen = SamsungServerSdk.readHealthDataBloodOxygen(context, days = 7)
val skinTemp = SamsungServerSdk.readHealthDataSkinTemperature(context, days = 7)
```

삼성 기기가 아니거나 삼성헬스 미설치/미로그인 등 사전 조건 미충족 시 `health.HealthDataSdkException(code: HealthDataErrorCode, ...)`을 던진다.
`samsung-health-data-api` AAR은 파트너 개별 배포라 git에는 pom만 있다 — 실물 `.aar`(원본 파일명 `samsung-health-data-api-1.1.0.aar` 그대로)는 `samsung-server-sdk/libs/repo/.../samsung-health-data-api/1.1.0/`에 직접 추가해야 빌드된다 (`samsung-server-sdk/libs/repo/README.md` 참고).
내부 구현은 `health/` 패키지(`HealthDataStoreProvider`, `HealthDataPermissionManager`, `*HealthDataReader`, `HealthDataManager`)에 있으며 전부 `internal` — `SamsungServerSdk`만 공개 API로 노출한다.
