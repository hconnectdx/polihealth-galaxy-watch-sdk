package kr.co.hconnect.polihealth_galaxy_watch_android_sdk.service
...
import kr.co.hconnect.polihealth_galaxy_watch_android_sdk.ble.NusConstants
import kr.co.hconnect.polihealth_galaxy_watch_android_sdk.ble.PacketReassembler
import kr.co.hconnect.polihealth_galaxy_watch_android_sdk.ble.WatchFinder
import kr.co.hconnect.polihealth_galaxy_watch_android_sdk.session.SessionManager
import kr.co.hconnect.polihealth_galaxy_watch_android_sdk.util.Constants
import kr.co.hconnect.polihealth_galaxy_watch_android_sdk.write.DataWriter
...
class WatchReceiverService : Service() {
    ...
    override fun onCreate() {
        ...
        val callback = PolihealthGalaxyWatchAndroidSdk.getCallback()
        if (callback == null) {
            Log.e(TAG, "PolihealthGalaxyWatchAndroidSdk.init() 미호출 상태에서 서비스 시작됨")
            stopSelf()
            return
        }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (watch == null) {
            ...
            PolihealthGalaxyWatchAndroidSdk.getCallback()?.onError("Galaxy Watch를 찾을 수 없습니다. 페어링 상태를 확인하세요.")
            stopSelf()
            return START_NOT_STICKY
        }
    ...
    private fun connectWatch(address: String) {
        ...
        val device = HCBle.getDevice(address) ?: run {
            ...
            PolihealthGalaxyWatchAndroidSdk.getCallback()?.onError("BluetoothDevice를 가져올 수 없습니다: $address")
            stopSelf()
            return
        }
            ...
            onConnState = { state ->
                when (state) {
                    BLEState.STATE_CONNECTED -> {
                        ...
                        PolihealthGalaxyWatchAndroidSdk.getCallback()?.onConnected(device.name ?: address)
                    }
