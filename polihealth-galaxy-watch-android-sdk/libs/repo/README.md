...
`samsung-sdk`의 `samsung-health-sensor-api-1.4.1.aar`와 동일하게 이 저장소의 기존 관례다).
빌드하려면 파트너로부터 받은 `samsung-health-data-api-1.1.0.aar` 파일(원본 파일명 그대로)을
아래 경로에 직접 추가해야 한다:

```
samsung-server-sdk/libs/repo/com/samsung/android/sdk/health/samsung-health-data-api/1.1.0/samsung-health-data-api-1.1.0.aar
```
