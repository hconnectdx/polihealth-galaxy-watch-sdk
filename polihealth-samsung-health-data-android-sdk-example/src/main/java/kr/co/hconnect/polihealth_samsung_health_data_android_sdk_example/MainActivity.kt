package kr.co.hconnect.polihealth_samsung_health_data_android_sdk_example
...
import kr.co.hconnect.polihealth_samsung_health_data_android_sdk.HealthDataSdkException
import kr.co.hconnect.polihealth_samsung_health_data_android_sdk.PolihealthSamsungHealthDataAndroidSdk
import kr.co.hconnect.polihealth_samsung_health_data_android_sdk_example.ui.theme.PolihealthSamsungHealthDataAndroidSdkTheme
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            PolihealthSamsungHealthDataAndroidSdkTheme {
                MainScreen()
            }
        }
    }
 ...
 * (polihealth-samsung-health-data-android-sdk/docs/samsung-health-integration.md 11절).
 */
...
fun MainScreen() {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Polihealth Samsung Health Data Android SDK Example") },
        ...
        Column(
            ...
            Row(
                ...
                HealthActionButton(
                    ...
                    runTask("권한 요청") {
                        val granted = PolihealthSamsungHealthDataAndroidSdk.requestHealthDataPermission(target)
                        permissionState = granted
            ...
            Row(
                ...
                HealthActionButton(text = "걸음수", modifier = Modifier.weight(1f), enabled = !loading) {
                    runTask("걸음수") {
                        PolihealthSamsungHealthDataAndroidSdk.readHealthDataSteps(context).map { "[걸음수] $it" }
                            .ifEmpty { listOf("[걸음수] 데이터 없음") }
                    }
                }
                HealthActionButton(text = "심박수", modifier = Modifier.weight(1f), enabled = !loading) {
                    runTask("심박수") {
                        PolihealthSamsungHealthDataAndroidSdk.readHealthDataHeartRate(context).map { "[심박수] $it" }
            ...
            Row(
                ...
                HealthActionButton(text = "산소포화도", modifier = Modifier.weight(1f), enabled = !loading) {
                    runTask("산소포화도") {
                        PolihealthSamsungHealthDataAndroidSdk.readHealthDataBloodOxygen(context).map { "[산소포화도] $it" }
                            .ifEmpty { listOf("[산소포화도] 데이터 없음") }
                    }
                }
                HealthActionButton(text = "피부온도", modifier = Modifier.weight(1f), enabled = !loading) {
                    runTask("피부온도") {
                        PolihealthSamsungHealthDataAndroidSdk.readHealthDataSkinTemperature(context).map { "[피부온도] $it" }
