...
import kr.co.hconnect.samsung_server_sdk.health.HealthDataSdkException

/**
 * 삼성헬스 Data SDK 연동(걸음수/심박수/산소포화도/피부온도) 수동 테스트 화면.
 *
 * 실제 삼성 기기 + 삼성헬스 앱(로그인된 상태)에서만 의미 있는 결과가 나온다
 * (samsung-server-sdk/docs/samsung-health-integration.md 11절).
 */
        ...
        Column(
            ...
            Row(
                ...
                HealthActionButton(
                    ...
                    runTask("권한 확인") {
                        val granted = SamsungServerSdk.checkHealthDataPermission(context)
                ...
                HealthActionButton(
                    ...
                    runTask("권한 요청") {
                        val granted = SamsungServerSdk.requestHealthDataPermission(target)
                        permissionState = granted
            ...
            Row(
                ...
                HealthActionButton(text = "걸음수", modifier = Modifier.weight(1f), enabled = !loading) {
                    runTask("걸음수") {
                        SamsungServerSdk.readHealthDataSteps(context).map { "[걸음수] $it" }
                            .ifEmpty { listOf("[걸음수] 데이터 없음") }
                    }
                }
                HealthActionButton(text = "심박수", modifier = Modifier.weight(1f), enabled = !loading) {
                    runTask("심박수") {
                        SamsungServerSdk.readHealthDataHeartRate(context).map { "[심박수] $it" }
            ...
            Row(
                ...
                HealthActionButton(text = "산소포화도", modifier = Modifier.weight(1f), enabled = !loading) {
                    runTask("산소포화도") {
                        SamsungServerSdk.readHealthDataBloodOxygen(context).map { "[산소포화도] $it" }
                            .ifEmpty { listOf("[산소포화도] 데이터 없음") }
                    }
                }
                HealthActionButton(text = "피부온도", modifier = Modifier.weight(1f), enabled = !loading) {
                    runTask("피부온도") {
                        SamsungServerSdk.readHealthDataSkinTemperature(context).map { "[피부온도] $it" }
