package kr.co.hconnect.polihealth_galaxy_watch_wearos_sdk

import android.content.Context
import kr.co.hconnect.polihealth_galaxy_watch_wearos_sdk.buffer.SensorBufferStorage
import kr.co.hconnect.polihealth_galaxy_watch_wearos_sdk.callback.SensorDataCallback
import kr.co.hconnect.polihealth_galaxy_watch_wearos_sdk.proto.SensorType
import kr.co.hconnect.polihealth_galaxy_watch_wearos_sdk.scheduler.AlarmScheduler
import kr.co.hconnect.polihealth_galaxy_watch_wearos_sdk.state.TrackingStateSender
import kr.co.hconnect.polihealth_galaxy_watch_wearos_sdk.tracker.HealthTrackerManager
import kr.co.hconnect.polihealth_galaxy_watch_wearos_sdk.tracker.HealthTrackerProcessorImpl
import kr.co.hconnect.polihealth_galaxy_watch_wearos_sdk.tracker.TrackerManager
import kr.co.hconnect.polihealth_galaxy_watch_wearos_sdk.tracker.TrackerProcessor
import kr.co.hconnect.polihealth_galaxy_watch_wearos_sdk.tracker.TrackingService
import kr.co.hconnect.polihealth_galaxy_watch_wearos_sdk.util.Constants
import kr.co.hconnect.polihealth_galaxy_watch_wearos_sdk.util.PreferencesUtil
import kotlinx.coroutines.runBlocking

/**
 * polihealth-galaxy-watch-wearos-sdk 라이브러리의 공개 진입점.
 *
 * 사용 방법:
 * ```
 * // 1) 초기화 — 앱 시작 시 한 번
 * PolihealthGalaxyWatchWearOsSdk.init(callback)
 *
 * // 2) 설정 — 주기 측정 파라미터 지정
 * PolihealthGalaxyWatchWearOsSdk.setMeasurementDuration(context, 120_000L)           // 측정 시간
 * PolihealthGalaxyWatchWearOsSdk.setAlarmSlotMinutes(context, intArrayOf(0, 30))     // 매 시 0분, 30분
 * PolihealthGalaxyWatchWearOsSdk.setAlarmSensorTypes(context, setOf(SensorType.ACC)) // 알람 시 측정할 센서
 *
 * // 3) 주기 측정 시작 (1회)
 * PolihealthGalaxyWatchWearOsSdk.startPeriodicTracking(context, durationMs, slotMinute, sensorTypes)
 *
 * // 4) 주기 알람 스케줄링 (반복)
 * PolihealthGalaxyWatchWearOsSdk.schedulePeriodicAlarm(context, sensorTypes)
 * PolihealthGalaxyWatchWearOsSdk.cancelPeriodicAlarm(context)
 *
 * // 5) 온디맨드 측정
 * PolihealthGalaxyWatchWearOsSdk.startOnDemandTracking(context, sensorTypes)
 * PolihealthGalaxyWatchWearOsSdk.stopOnDemandTracking(context)
 * ```
 */
object PolihealthGalaxyWatchWearOsSdk {
    ...
    fun startPeriodicTracking(
        ...
        check(initialized) { "PolihealthGalaxyWatchWearOsSdk.init()을 먼저 호출하세요." }
