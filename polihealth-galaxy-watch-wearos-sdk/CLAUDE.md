...
`PolihealthGalaxyWatchWearOsSdk` (object) — 유일한 공개 진입점.

```kotlin
// 필수: 앱 시작 시 한 번
PolihealthGalaxyWatchWearOsSdk.init(callback: SensorDataCallback)

// 설정 (DataStore 저장, 재부팅 후 ScheduleReceiver가 읽음)
PolihealthGalaxyWatchWearOsSdk.setMeasurementDuration(context, durationMs)
PolihealthGalaxyWatchWearOsSdk.setAlarmSlotMinutes(context, intArrayOf(0, 30))   // 기본: [1, 31]
PolihealthGalaxyWatchWearOsSdk.setAlarmSensorTypes(context, setOf(SensorType.ACC))
...
// 즉시 측정 (수동 시작/정지)
PolihealthGalaxyWatchWearOsSdk.startOnDemandTracking(context, sensorTypes, timestamp)
PolihealthGalaxyWatchWearOsSdk.stopOnDemandTracking(context)

// 상태 관찰
PolihealthGalaxyWatchWearOsSdk.trackingState: StateFlow<SdkTrackingState>  // Idle/Start/Finish
```

## 아키텍처

```
PolihealthGalaxyWatchWearOsSdk (object)
